...
async def nodes_children_scan(
    node: ua.Node, client: ua.Client, default_value=True
) -> Tuple[Dict, str]:
    async def recursive_children_scan(node: ua.Node) -> Tuple[Dict, str]:
        ...
        if sub_nodes:
            for node in sub_nodes:
                child_node_attrs, child_node_id = await recursive_children_scan(
                    node=node
                )
                node_struct[node_id][stc.CHILDREN][child_node_id] = child_node_attrs[
                    child_node_id
                ]
...
async def nodes_parent_scan(node: ua.Node, client: ua.Client, default_value=True):
    async def recursive_parent_scan(
        node: ua.Node, child_node_attrs: Dict = {}, child_node_id: str = ""
    ) -> Tuple[Dict, str]:
        node_struct, node_id = await get_node_attrs(node, client, default_value)
        if child_node_attrs:
            node_struct[node_id][stc.CHILDREN] = {
                child_node_id: child_node_attrs[child_node_id]
            }
        else:
            node_struct[node_id][stc.CHILDREN] = {}
        parent_node = await node.get_parent()
        if parent_node:
            parent_node_attrs, parent_node_id = await recursive_parent_scan(
                parent_node, node_struct, node_id
            )
...
async def create_root_to_udt_struct(
    udt_entry_index: List[str], client: ua.Client, udt_structs: List[Dict]
):
    nodes_struct = []
