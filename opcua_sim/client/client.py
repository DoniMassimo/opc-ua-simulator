...
async def build_structure(udt_entry_index: List[str], client: ua.Client, default_values: bool) -> Dict:
    udt_to_leaf_struct = []
    ...
    server_structure = await nm.create_root_to_udt_struct(udt_entry_index, client, udt_to_leaf_struct) 
    return server_structure

def start_client(config_path: str, save_dir: str):
...
async def main(config: Dict, save_dir: str):       
    for settings in config[cnc.OPC_SETTINGS]:
        ...
        async with ua.Client(url=url) as client:
            ...
            current_server_structure = await build_structure(udt_entry_path, client, default_value)
            server_structure[url] = current_server_structure
        utils.save_structure(server_structure, os.path.join(save_dir, save_file_name))

def test():
    save_dir = os.path.normpath(r'C:\Users\mdoni\Desktop\MAX\Project\Python\opc-ua-simulator\opcua_sim\Client\save')
    start_client(r'C:\Users\mdoni\Desktop\MAX\Project\Python\opc-ua-simulator\opcua_sim\Client\config.json', save_dir) 
