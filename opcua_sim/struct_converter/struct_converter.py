    ...
    def recursive_conversion(rel_struct: Dict):
        ...
        for key in rel_struct.keys():
            ...
            if rel_struct[key][gc.Struct.CHILDREN]:
                ...
                rel_conversion[gc.Ignition.TAGS] = recursive_conversion(
                    rel_struct[key][gc.Struct.CHILDREN]
                )
            else:
                rel_conversion[gc.Ignition.VALUE_SOURCE] = "opc"
                rel_conversion[gc.Ignition.OPC_ITEM_PATH] = rel_struct[key][
                    gc.Struct.NODE_ID
                ]
...
def start_conversion(struct: Dict, entry_index: List[str]):
    ...
    utils.save_structure(
        data,
        r"C:\Users\mdoni\Desktop\MAX\Project\Python\opc-ua-simulator\opcua_sim\Client\save\data.json",
    )
