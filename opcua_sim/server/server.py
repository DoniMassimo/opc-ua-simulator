...
async def get_node_attrs(node, default_value=True):
    ...
    node_id = (
        f"ns={attrs[2].Value.Value.NamespaceIndex};i={attrs[2].Value.Value.Identifier}"
    )
    ...
    attrs_dict = {
        node_id: {
            sc.BROWSE_NAME: browse_name,
            sc.NODE_ID: node_id,
            sc.NODE_CLASS: node_class,
            sc.VALUE: value,
...
async def create_node(node_attrs: Dict[str, str], parent_node: ua.Node):
    new_node: ua.Node
    if node_attrs[sc.NODE_CLASS] == 1:
        new_node = await parent_node.add_object(
            node_attrs[sc.NODE_ID], node_attrs[sc.DISPLAY_NAME]
        )
    else:
        new_node = await parent_node.add_variable(
            node_attrs[sc.NODE_ID], node_attrs[sc.DISPLAY_NAME], node_attrs[sc.VALUE]
        )
        await new_node.set_writable()
    return new_node


async def create_node_from_struct(
    rel_server_struct: Dict, current_node: ua.Node, server: ua.Server
):
    struct_id = list(rel_server_struct.keys())[0]
    rel_struct_attrs = rel_server_struct[struct_id]
    if rel_struct_attrs[sc.CHILDREN]:
        current_childer_id = await get_node_children_id(current_node)
        for child_id, child_attrs in rel_struct_attrs[sc.CHILDREN].items():
...
def start_server(project_path, data_change_callback: Dict[str, Callable]):
    config_path = os.path.join(project_path, "server/config.json")
    server_config = utils.load_config(config_path)
    struct_file = server_config[sc.STRUCT_FILE]
    struct_path = os.path.join(project_path, "struct", struct_file)
    server_structure, server_endpoint = get_struct_and_endpoint(struct_path)
    print("prinma di asincio")
...
async def main(
    ...
    async with server:
        while True:
            ...
            i += 0.6
            print("eseguendo... ", server_endpoint)
